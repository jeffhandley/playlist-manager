...
- **Sync Playlists** — Use the **sync-playlists** skill to sync one or all playlists to Apple Music via the API. A GitHub Actions workflow runs automatically on push to `main`, or trigger manually with `gh workflow run "Sync Playlists to Apple Music"`.
- **Validate URLs** — Use the **validate-urls** skill to check all Apple Music permalinks and fix stale/broken ones. Runs the validation script directly and creates a PR with corrections if any are found.
...
| **validate-urls** | Validate and fix Apple Music permalinks, creating a PR with corrections |
